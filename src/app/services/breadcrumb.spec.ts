...
function createMockRoute(config?: {
  ...
  if (config?.url) {
    (route as any).url = config.url.split('/').filter(Boolean).map(path => ({ path }));
  } else {
    (route as any).url = [];
  }
  
  (route as any).data = config?.data || {};
  (route as any).params = config?.params || {};
  (route as any).queryParams = config?.queryParams || {};
  
  if (config?.children && config.children.length > 0) {
    (route as any).firstChild = config.children[0];
  } else {
    (route as any).firstChild = null;
  }
